...
impl IntegerValidator {
    ...
    pub fn parse_f64(&self, value: f64) -> IntegerResult {
        if (value.floor() - value).abs() >= 1.0e-6 {
            return Err(IntegerError::UnpreciseError);
        }
    ...
    pub fn parse_f32(&self, value: f32) -> IntegerResult {
        if (value.floor() as f32 - value).abs() >= 1.0e-6 {
            return Err(IntegerError::UnpreciseError);
        }
    ...
    pub fn parse_i8(&self, value: i8) -> IntegerResult {
        self.parse_i128(i128::from(value))
    }
    ...
    fn parse_inner(&self, full_integer: &str) -> IntegerResult {
        let value =
            full_integer.parse::<f64>().map_err(|err| IntegerError::ParseError(err.to_string()))?;
...
macro_rules! extend {
    ($name:ident, $negative:expr, $zero:expr) => {
        ...
        impl std::str::FromStr for $name {
            ...
            #[inline]
            fn from_str(s: &str) -> Result<$name, IntegerError> {
                $name::from_str(s)
            }
        }
            ...
            where
                ...
                impl<'de> ::serde::de::Visitor<'de> for IntegerVisitor {
                    ...
                    fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
                        formatter
                            .write_fmt(format_args!("a integer({:?})", $name::create_validator()))
                    }
                    ...
                    where
                        E: ::serde::de::Error, {
                        $name::from_i128(v).map_err(|err| E::custom(err.to_string()))
                    }
                }
