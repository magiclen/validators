...
const VALIDATOR: Validator = Validator::base64;

pub fn base64_handler(ast: DeriveInput, meta: Meta) -> TokenStream {
    match ast.data {
        Data::Struct(data) => {
            if let Fields::Unnamed(_) = &data.fields {
                ...
                let correct_usage_for_attribute = [stringify!(#[validator(base64)])];

                let correct_usage_for_padding = [
                    stringify!(#[validator(base64(padding(Must)))]),
                    stringify!(#[validator(base64(padding(Allow)))]),
                    stringify!(#[validator(base64(padding(NotAllow)))]),
                ];
                ...
                match meta {
                    ...
                    Meta::NameValue(_) => {
                        panic::attribute_incorrect_format("base64", &correct_usage_for_attribute)
                    }
                }
                ...
                let error_path: Path =
                    syn::parse2(quote! { validators_prelude::base64::Base64Error }).unwrap();
