...
const VALIDATOR: Validator = Validator::semver_req;

pub fn semver_req_handler(ast: DeriveInput, meta: Meta) -> TokenStream {
    match ast.data {
        Data::Struct(data) => {
            if let Fields::Unnamed(_) = &data.fields {
                ...
                let correct_usage_for_attribute = [stringify!(#[validator(semver_req)])];
                    ...
                    quote! {
                        ...
                        impl<'de> validators_prelude::Deserialize<'de> for #name {
                            ...
                            where
                                ...
                                Ok(#name(validators_prelude::semver::VersionReq::deserialize(deserializer)?))
                            }
                        }
                    }
                } else {
                    ...
                    quote! {
                        impl<'a> validators_prelude::FromFormValue<'a> for #name {
                            ...
                            fn from_form_value(v: &'a validators_prelude::RawStr) -> Result<Self, Self::Error> {
                                <#name as ValidateString>::parse_string(v.url_decode().map_err(|_| #error_path(validators_prelude::semver::ReqParseError::InvalidIdentifier))?)
                            }
                        }
