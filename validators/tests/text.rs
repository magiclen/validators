    ...
    macro_rules! test {
        ($( { $( $p:meta => $v:expr ),* $(,)* } ),* $(,)* ) => {
            $(
                {
                    #[derive(Validator)]
                    #[validator(text(char_length($($p = $v,)*)))]
                    ...
                    test("123\n456", check_char_length("123\n456", Validator::V_CHAR_LENGTH_MIN, Validator::V_CHAR_LENGTH_MAX, Validator::V_CHAR_LENGTH_TRIMMED_MIN));
                }
            )*
        }
    }
    ...
    macro_rules! test {
        ($( { $( $p:meta => $v:expr ),* $(,)* } ),* $(,)* ) => {
            $(
                {
                    #[derive(Validator)]
                    #[validator(text(byte_length($($p = $v,)*)))]
                    pub struct Validator(pub String);
                    ...
                    test("123\n456", check_byte_length("123\n456", Validator::V_BYTE_LENGTH_MIN, Validator::V_BYTE_LENGTH_MAX, Validator::V_BYTE_LENGTH_TRIMMED_MIN));
                }
            )*
        }
    }
