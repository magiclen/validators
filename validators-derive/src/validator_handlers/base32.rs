...
const VALIDATOR: Validator = Validator::base32;

pub fn base32_handler(ast: DeriveInput, meta: Meta) -> TokenStream {
    match ast.data {
        Data::Struct(data) => {
            if let Fields::Unnamed(_) = &data.fields {
                ...
                let correct_usage_for_attribute = [stringify!(#[validator(base32)])];

                let correct_usage_for_padding = [
                    stringify!(#[validator(base32(padding(Must)))]),
                    stringify!(#[validator(base32(padding(Allow)))]),
                    stringify!(#[validator(base32(padding(NotAllow)))]),
                ];
                ...
                match meta {
                    ...
                    Meta::NameValue(_) => {
                        panic::attribute_incorrect_format("base32", &correct_usage_for_attribute)
                    }
                }
