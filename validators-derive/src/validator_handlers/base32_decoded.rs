...
const VALIDATOR: Validator = Validator::base32_decoded;

pub fn base32_decoded_handler(ast: DeriveInput, meta: Meta) -> TokenStream {
    match ast.data {
        Data::Struct(data) => {
            if let Fields::Unnamed(_) = &data.fields {
                ...
                let correct_usage_for_attribute = [stringify!(#[validator(base32_decoded)])];

                let correct_usage_for_padding = [
                    stringify!(#[validator(base32_decoded(padding(Must)))]),
                    stringify!(#[validator(base32_decoded(padding(Allow)))]),
                    stringify!(#[validator(base32_decoded(padding(NotAllow)))]),
                ];
                ...
                let error_path: Path =
                    syn::parse2(quote! { validators_prelude::base32_decoded::Base32DecodedError })
                        .unwrap();
                ...
                let serde_impl = if cfg!(feature = "serde") {
                    let expect = match padding {
                        ...
                        ValidatorOption::Must => "a Base32 string or data with padding",
                        ValidatorOption::NotAllow => "a Base32 string or data without padding",
                    };
