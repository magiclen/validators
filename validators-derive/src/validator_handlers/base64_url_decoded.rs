...
const VALIDATOR: Validator = Validator::base64_url_decoded;

pub fn base64_url_decoded_handler(ast: DeriveInput, meta: Meta) -> TokenStream {
    match ast.data {
        Data::Struct(data) => {
            if let Fields::Unnamed(_) = &data.fields {
                ...
                let correct_usage_for_attribute = [stringify!(#[validator(base64_url_decoded)])];

                let correct_usage_for_padding = [
                    stringify!(#[validator(base64_url_decoded(padding(Must)))]),
                    stringify!(#[validator(base64_url_decoded(padding(Allow)))]),
                    stringify!(#[validator(base64_url_decoded(padding(NotAllow)))]),
                ];
                ...
                let error_path: Path = syn::parse2(
                    quote! { validators_prelude::base64_url_decoded::Base64UrlDecodedError },
                )
                .unwrap();
                ...
                let base64_url_decoded_impl = quote! {
                    #parameters_impl
